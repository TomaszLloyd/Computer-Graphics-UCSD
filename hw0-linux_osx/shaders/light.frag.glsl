...
{       
    if (istex > 0) fragColor = texture(tex, texcoord); 
        ...
        vec4 col0 = ComputeLight(direction, lightcolor, normal, halfVec, diffuse, specular, shininess) ;

        // Light 1, point 
        vec3 position = light1posn.xyz / light1posn.w ; 
        vec3 direction1 = normalize (position - mypos) ; // no attenuation 
        vec3 half1 = normalize (direction1 + eyedirn) ;  
        vec4 col1 = ComputeLight(direction, lightcolor, normal, halfVec, diffuse, specular, shininess) ;
